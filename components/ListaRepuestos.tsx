...
interface ListaMarcasProps {
  marcas: Marca[];
  isLoading?: boolean; // para estado de carga (opcional)
}

// Componente interno para cada marca (memoizado)
      ...
      <div className="w-full h-28 bg-gradient-to-br from-gray-50 to-gray-100/70 rounded-xl flex items-center justify-center p-4 mb-3 transition-colors duration-300 group-hover:from-blue-50/30 group-hover:to-blue-100/20 overflow-hidden">
        {hasImage ? (
          ...
          <div className="flex flex-col items-center justify-center text-gray-400 gap-1.5">
            ...
            <span className="text-[10px] font-medium tracking-wider uppercase text-gray-400">Sin logo</span>
          </div>
        )}
      </div>
...
export default function ListaMarcas({ marcas, isLoading = false }: ListaMarcasProps) {
  // ✅ Verificación segura incluso si marcas es undefined
  ...
  if (sortedMarcas.length === 0) {
    return (
      <div className="text-center p-12 border-2 border-dashed border-gray-200 rounded-2xl bg-gray-50/50">
        ...
        <p className="text-gray-400 text-sm font-medium">No encontramos marcas disponibles.</p>
